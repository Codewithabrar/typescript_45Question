...
let current_users: string[] = ['zakirya','qamar','aneel','dilawar','mansoor'];
let new_users: string[] = ['abrar','ali','zakirya','aneel','mushahid'];
new_users.forEach(
    (new_users)=>{
        if(current_users.some((current_users)=> current_users===new_users)){
            console.log(`${new_users} the person will need to enter a new username`)
        }else{
